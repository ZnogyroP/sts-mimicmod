...
import themimic.cards.Basic.Confound;
import themimic.cards.Basic.Defend;
import themimic.cards.Basic.Opportunist;
import themimic.cards.Basic.Strike;
import themimic.relics.StrangeSap;

import java.util.ArrayList;

import static themimic.TheMimicMod.characterPath;
import static themimic.TheMimicMod.makeID;

public class MimicCharacter extends CustomPlayer {
    //Stats
    ...
    //Strings
    private static final String ID = makeID("Mimic"); //This should match whatever you have in the CharacterStrings.json file
    ...
    public static class Meta {
        ...
        @SpireEnum
        public static PlayerClass MIMIC_CHAR;
        ...
        private static final Color cardColor = new Color(Color.YELLOW);
        ...
        public static void registerCharacter() {
            BaseMod.addCharacter(new MimicCharacter(), CHAR_SELECT_BUTTON, CHAR_SELECT_PORTRAIT);
        }
    }
    ...
    public MimicCharacter() {
        super(getNames()[0], Meta.MIMIC_CHAR,
                new CustomEnergyOrb(orbTextures, characterPath("energyorb/vfx.png"), layerSpeeds), //Energy Orb
                new SpineAnimation(characterPath("skeleton.atlas"), characterPath("skeleton.json"), 0.8f)); //Animation
        ...
        AnimationState.TrackEntry e = state.setAnimation(0, "default", true);
    ...
    public ArrayList<String> getStartingDeck() {
        ...
        retVal.add(Defend.ID);
        retVal.add(Defend.ID);
        retVal.add(Opportunist.ID);
        retVal.add(Confound.ID);
    ...
    private final Color cardRenderColor = Color.GOLDENROD.cpy(); //Used for some vfx on moving cards (sometimes) (maybe)
    private final Color cardTrailColor = Color.GOLDENROD.cpy(); //Used for card trail vfx during gameplay.
    private final Color slashAttackColor = Color.GOLDENROD.cpy(); //Used for a screen tint effect when you attack the heart.
    @Override
