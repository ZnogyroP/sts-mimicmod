...
public class InvasiveTendrils extends BaseCard {
    public static final String ID = makeID(InvasiveTendrils.class.getSimpleName()); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        for(AbstractMonster mo : AbstractDungeon.getCurrRoom().monsters.monsters) {
            if (mo.hasPower(LeechPower.POWER_ID)) {
                addToBot(new LeechLoseHpAction(mo, p, mo.getPower(LeechPower.POWER_ID).amount, AbstractGameAction.AttackEffect.POISON));
            }
        }
    }
}
