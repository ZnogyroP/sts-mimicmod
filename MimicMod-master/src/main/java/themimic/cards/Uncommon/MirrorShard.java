...
import themimic.actions.StealBlockAction;
import themimic.cards.BaseCard;
import themimic.character.MimicCharacter;
import themimic.util.CardStats;

public class MirrorShard extends BaseCard {
    public static final String ID = makeID(MirrorShard.class.getSimpleName()); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        if (m.currentBlock > 0) {
            addToBot(new StealBlockAction(m, p));
        } else {
