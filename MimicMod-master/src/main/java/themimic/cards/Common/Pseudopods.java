...
public class Pseudopods extends BaseCard {
    public static final String ID = makeID(Pseudopods.class.getSimpleName()); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    public Pseudopods() {
        ...
        this.cardsToPreview = new Slimed().makeCopy();
    }
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        addToBot(new DamageAction(m, new DamageInfo(p, damage, DamageInfo.DamageType.NORMAL), AbstractGameAction.AttackEffect.BLUNT_LIGHT));
        addToBot(new MakeTempCardInDiscardAction(new Slimed().makeCopy(), 2));
    }
