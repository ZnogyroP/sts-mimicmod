...
public class LashOut extends BaseCard {
    public static final String ID = makeID(LashOut.class.getSimpleName()); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    public LashOut() {
        ...
        setCustomVar("weakDamage", VariableType.DAMAGE, 24, 8);
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        if (p.hasPower(WeakPower.POWER_ID)) {
            addToBot(new DamageAction(m, new DamageInfo(p, customVar("weakDamage"), DamageInfo.DamageType.NORMAL), AbstractGameAction.AttackEffect.BLUNT_HEAVY));
        } else {
