...
public class Tilt extends BaseCard {
    public static final String ID = makeID(Tilt.class.getSimpleName()); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    public Tilt() {
        ...
        setCustomVar("dexNumber", 1, 0); //Sets the card's damage and how much it changes when upgraded.
    }

    @Override
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p, p, new StrengthPower(p, magicNumber), magicNumber));
        addToBot(new ApplyPowerAction(p, p, new DexterityPower(p, -customVar("dexNumber")), -customVar("dexNumber")));
    }
