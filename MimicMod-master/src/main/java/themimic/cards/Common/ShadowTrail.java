...
import themimic.actions.ShadowTrailAction;
import themimic.cards.BaseCard;
import themimic.character.MimicCharacter;
import themimic.util.CardStats;

public class ShadowTrail extends BaseCard {
    public static final String ID = makeID(ShadowTrail.class.getSimpleName()); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        addToBot(new ShadowTrailAction(m, new DamageInfo(p, damage, DamageInfo.DamageType.NORMAL)));
    }
